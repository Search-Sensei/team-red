...
class OidcProviderSettings
{
    ...
    protected function validateInitial(): void
    {
        ...
        // LOCAL-DEV PATCH: relaxed to allow http:// so local Keycloak (on host.docker.internal:8080)
        // can be used without setting up TLS. Production uses https:// and passes either branch.
        // See docs/bookstack-integration.md -> "Local development notes".
        if (!str_starts_with($this->issuer, 'https://') && !str_starts_with($this->issuer, 'http://')) {
            throw new InvalidArgumentException('Issuer value must start with http:// or https://');
        }
    }
    ...
    public function validate(): void
    {
        ...
        // LOCAL-DEV PATCH: endpoints discovered from the issuer may also be http:// in local setups.
        // Same reasoning as the issuer patch in validateInitial(). See docs/bookstack-integration.md.
        ...
        foreach ($endpointProperties as $prop) {
            if (is_string($this->$prop)
                && !str_starts_with($this->$prop, 'https://')
                && !str_starts_with($this->$prop, 'http://')) {
                throw new InvalidArgumentException("Endpoint value for \"{$prop}\" must start with http:// or https://");
            }
        }
    }
