    ...
    public class KeycloakService
    {
        ...
        )
        {
            ...
            if (!_isEnabled)
            {
                _logger.LogInformation(
                    "KeycloakService initialized in bypass mode because KeycloakAuthentication:IsEnabled is false."
                );
                return;
            }
            ...
            if (string.IsNullOrWhiteSpace(baseUrl))
            {
                throw new InvalidOperationException(
                    "KeycloakAuthentication:BaseUrl is required when Keycloak authentication is enabled."
                );
            }
            ...
            _httpClient.DefaultRequestHeaders.Add(
                "x-api-key",
                _configuration["KeycloakAuthentication:ApiKey"] ?? "12345-ABCDE-67890"
            );
        }
        ...
        public async Task<string> GetAdminTokenAsync()
        {
            ...
            if (string.IsNullOrWhiteSpace(realm))
                throw new InvalidOperationException(
                    "Cannot find Keycloak realm. Configuration missing."
                );
            ...
            if (string.IsNullOrWhiteSpace(adminClientId))
                throw new InvalidOperationException(
                    "KeycloakAuthentication:AdminClientId is required for admin token flow."
                );

            if (string.IsNullOrWhiteSpace(adminClientSecret))
                throw new InvalidOperationException(
                    "KeycloakAuthentication:AdminClientSecret is required for admin token flow."
                );
        ...
        public async Task<SwitchContextResponse> RefreshTokenAsync(string refreshToken)
        {
            ...
            if (string.IsNullOrWhiteSpace(refreshClientId))
                throw new InvalidOperationException(
                    "KeycloakAuthentication:ClientId is required for refresh token flow."
                );
            ...
            {
                formData.Add(
                    new KeyValuePair<string, string>("client_secret", refreshClientSecret)
                );
            }
            ...
            var requestInfo = new
            {
                ...
                Headers = _httpClient.DefaultRequestHeaders.ToDictionary(
                    h => h.Key,
                    h => h.Value.ToArray()
                ),
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError(
                    "RefreshTokenAsync: Failed to refresh token - Status: {Status}, Error: {Error}",
                    response.StatusCode,
                    errorContent
                );
                response.EnsureSuccessStatusCode();
            ...
            return new SwitchContextResponse
            {
                ...
                ExpiresIn = int.TryParse(tokenData?["expires_in"]?.ToString(), out var ei)
                    ? ei
                    : 300,
                RefreshExpiresIn = int.TryParse(
                    tokenData?["refresh_expires_in"]?.ToString(),
                    out var rei
                )
                    ? rei
                    : 0,
                ...
                NotBeforePolicy = int.TryParse(
                    tokenData?["not-before-policy"]?.ToString(),
                    out var nbp
                )
                    ? nbp
                    : 0,
                ...
                IdToken =
                    tokenData?.ContainsKey("id_token") == true
                        ? tokenData["id_token"]?.ToString()
                        : null,
            };
        }
        ...
        public async Task UpdateActiveTenantAsync(
            string adminToken,
            string userId,
            string activeTenant
        )
        {
            ...
            var user =
                System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, object>>(userJson)
            ...
            var url =
                $"/admin/realms/{_configuration["KeycloakAuthentication:Realm"]}/users/{userId}";
        ...
        )
        {
            ...
            var orgPayload = new
            {
                name = name,
                enabled = true,
                domains = new[] { new { name = domainName, verified = false } },
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError(
                    "Failed to create organization: {Status} {Error}",
                    response.StatusCode,
                    error
                );
                throw new Exception(
                    $"Failed to create organization: {response.StatusCode} - {error}"
                );
            }
            ...
            if (string.IsNullOrEmpty(locationHeader))
                throw new Exception(
                    "Keycloak did not return a Location header for the created organization."
                );
        ...
        public async Task AddUserToOrganizationAsync(string adminToken, string orgId, string userId)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to add user {UserId} to organization {OrgId}: {Status} {Error}",
                    userId,
                    orgId,
                    response.StatusCode,
                    error
                ...
                throw new Exception(
                    $"Failed to add user to organization: {response.StatusCode} - {error}"
                );
            }
        }
        ...
        public async Task DeleteOrganizationAsync(string adminToken, string orgId)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogWarning(
                    "Rollback: Failed to delete organization {OrgId}: {Status} {Error}",
                    orgId,
                    response.StatusCode,
                    error
                );
        ...
        public async Task DeleteUserAsync(string adminToken, string userId)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogWarning(
                    "Rollback: Failed to delete user {UserId}: {Status} {Error}",
                    userId,
                    response.StatusCode,
                    error
                );
        ...
        public async Task<List<TenantInfo>> GetUserOrganizationsAsync(
            string adminToken,
            string userId
        )
        {
        ...
        {
            if (
                org.TryGetProperty("attributes", out var attrs)
        ...
        public async Task<(string Id, string Name)> GetClientRoleAsync(
            string adminToken,
            string clientUuid,
            string roleName
        )
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to get client role '{RoleName}': {Status} {Error}",
                    roleName,
                    response.StatusCode,
                    error
                );
                throw new Exception(
                    $"Failed to get client role '{roleName}': {response.StatusCode} - {error}"
                );
            }
        ...
        public async Task AssignClientRoleToUserAsync(
            string adminToken,
            string userId,
            string clientUuid,
            string roleId,
        ...
        )
        {
            ...
            var rolePayload = new[] { new { id = roleId, name = roleName } };
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to assign client role '{RoleName}' to user {UserId}: {Status} {Error}",
                    roleName,
                    userId,
                    response.StatusCode,
                    error
                ...
                throw new Exception(
                    $"Failed to assign client role to user: {response.StatusCode} - {error}"
                );
            }
        }
        ...
        public async Task<SwitchContextResponse> SwitchContextAsync(SwitchContextRequest request)
        {
            ...
            if (
                ...
                || string.IsNullOrWhiteSpace(request.ActiveTenant)
            )
            {
                throw new ArgumentException(
                    "Refresh_token or active_tenant cannot be null or empty"
                );
            }
        ...
        public async Task<string?> GetOrganizationIdByNameAsync(string adminToken, string orgName)
        {
            ...
            var url =
                $"/admin/realms/{realm}/organizations?search={Uri.EscapeDataString(orgName)}&exact=true";
            _httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
                "Bearer",
                adminToken
            );
        ...
        public async Task InviteUserToOrganizationAsync(
            string adminToken,
            string orgId,
            string email,
            string firstName,
        ...
        )
        {
            ...
            var content = new FormUrlEncodedContent(
                [new("email", email), new("firstName", firstName), new("lastName", lastName)]
            );
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to invite user {Email} to organisation {OrgId}: {Status} {Error}",
                    email,
                    orgId,
                    response.StatusCode,
                    error
                ...
                if ((int)response.StatusCode == 409)
                    throw new KeycloakConflictException(
                        $"User {email} is already a member of the organisation."
                    );
        ...
        )
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to create invited user {Email}: {Status} {Error}",
                    email,
                    response.StatusCode,
                    error
                );
                ...
                throw new Exception(
                    $"Failed to create invited user: {response.StatusCode} - {error}"
                );
            }
            ...
            if (string.IsNullOrEmpty(locationHeader))
                throw new Exception(
                    "Keycloak did not return a Location header for the invited user."
                );
        ...
        )
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to send execute-actions email to user {UserId}: {Status} {Error}",
                    userId,
                    response.StatusCode,
                    error
                );
                throw new Exception(
                    $"Failed to send invite email: {response.StatusCode} - {error}"
                );
            }
        }
        ...
        /// </summary>
        public async Task<List<string>> GetExpiredInvitedUserIdsAsync(
            string adminToken,
            TimeSpan expiry
        )
        {
            ...
            foreach (var userEl in doc.RootElement.EnumerateArray())
            {
                ...
                if (
                    !attrs.TryGetProperty("invitedAt", out var invitedAtArr)
                ...
                if (
                    !userEl.TryGetProperty("requiredActions", out var actions)
                    || actions.ValueKind != JsonValueKind.Array
                )
                    continue;

                var stillPending = actions
                    .EnumerateArray()
        ...
        public async Task EnableUserAsync(string adminToken, string userId)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                var error = await response.Content.ReadAsStringAsync();
                _logger.LogError(
                    "Failed to enable user {UserId}: {Status} {Error}",
                    userId,
                    response.StatusCode,
                    error
                );
        ...
        )
        {
            ...
            if (!putResponse.IsSuccessStatusCode)
            {
                ...
                throw new Exception(
                    $"Failed to update org Stripe attributes: {putResponse.StatusCode} - {error}"
                );
            }
        ...
        )
        {
            ...
            while (true)
            {
                ...
                foreach (var userEl in users)
                {
                    ...
                    if (
                        !attrs.TryGetProperty("pendingPaymentSince", out var sinceArr)
                    ...
                    var orgId = "";
                    if (
                        attrs.TryGetProperty("pendingOrgId", out var orgArr)
